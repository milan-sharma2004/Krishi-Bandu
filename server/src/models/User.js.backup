...
export default function Users() {
  ...
  // Add this function after toggleStatus.
  async function approveUser(user) {
    try {
      await api.patch(
        `/admin/users/${user._id}/approval`,
        {
      ...
      notify(
        `${user.name}'s account has been approved.`,
        'success'
      );
  ...
  // Add this function below approveUser.
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.slice(0, 100).map((user) => {
              // Existing users may not have approvalStatus yet.
                  ...
                  <td className="px-4 py-3">
                    {user.role !== 'admin' && (
                      <div className="flex flex-wrap items-center gap-3">
                        {/* Approve and Reject only appear for pending users */}
                        {approvalStatus ===
                          'pending' && (
                          <>
                            <button
